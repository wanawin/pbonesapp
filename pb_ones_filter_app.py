...
def eval_bool(expr_code, ctx):
    """Evaluate expression and force a strict boolean result.
    Non-boolean returns (e.g., 1, 'True', 0.0) are treated as False to avoid 'always-true' wipes.
    """
    try:
...
def main():
    st.sidebar.header("🎯 Powerball Ones-Only — Manual Filter Runner (v6 strict)")
    ...
    for tok in toks:
        ...
        if len(digits_only) == 5:
            normalized.append(''.join(sorted(digits_only)))
    normalized_tracked = sorted(set(normalized))
    ...
    # Initial counts with strict booleans
