...
def load_filters_df():
    # Prefer single-file bundle; else fall back to split files
    candidates = [
    ...
    # Generate fallback IDs for blanks, then deduplicate by id
    ids = []
    for i, fid in enumerate(df["id"].astype(str).tolist()):
        fid = (fid or "").strip()
        if not fid:
            fid = f"AUTOID_{i:04d}"
        ids.append(fid)
...
def main():
    st.sidebar.header("🎯 Powerball Ones-Only — Manual Filter Runner (dedup-safe)")
    ...
    # Compile & dedup filters
    filters = []
    for i, r in filters_df.reset_index(drop=True).iterrows():
        fid = (r.get("id","") or "").strip() or f"AUTOID_{i:04d}"
        ...
        try:
            ...
            continue
    # Make a dict to drop any duplicate ids that may have slipped through
    uniq = {}
    for tup in filters:
        fid = tup[0]
        if fid not in uniq:
            uniq[fid] = tup
    filters = list(uniq.values())
    ...
    for idx, (fid, name, app, expr, app_c, expr_c, en) in enumerate(display_filters):
        ...
        chk = st.checkbox(f"{fid}: {name} — init cuts {init_counts[fid]}", value=(en and select_all), key=key)
        if chk:
            survivors = []
            for combo in pool:
                ctx = build_ctx(seed, prev, prevprev, combo, hot_input, cold_input, due_set)
